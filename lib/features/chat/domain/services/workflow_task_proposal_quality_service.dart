...
class WorkflowTaskProposalQualityService {
  WorkflowTaskProposalQualityService({
    WorkflowTaskProposalIdGenerator? createId,
  }) : _createId = createId ?? _defaultCreateId;

  final WorkflowTaskProposalIdGenerator _createId;

  static String _defaultCreateId() => const Uuid().v4();

  bool isReasoningWorkflowProposalPlausible(WorkflowProposalDraft proposal) {
    return _isReasoningWorkflowProposalPlausible(proposal);
  }

  bool isReasoningTaskProposalPlausible(WorkflowTaskProposalDraft proposal) {
    return _isReasoningTaskProposalPlausible(proposal);
  }

  WorkflowTaskProposalDraft? preferTaskProposalRetryCandidate({
    required WorkflowTaskProposalDraft? current,
    required WorkflowTaskProposalDraft candidate,
  }) {
    return _preferTaskProposalRetryCandidate(
      current: current,
      candidate: candidate,
    );
  }

  List<ConversationWorkflowTask> buildHeuristicTaskProposalFallbackTasks({
    required List<String> contextLines,
    required bool projectLooksEmpty,
  }) {
    return _buildHeuristicTaskProposalFallbackTasks(
      contextLines: contextLines,
      projectLooksEmpty: projectLooksEmpty,
    );
  }

  WorkflowTaskProposalDraft finalizeTaskProposalDraft(
  ...
  List<ConversationWorkflowTask> sanitizeTaskProposalTasks(
    Iterable<ConversationWorkflowTask> tasks,
  ) {
    return _sanitizeTaskProposalTasks(tasks);
  }

  bool taskProposalNeedsRetry(
  ...
  bool taskProposalNeedsRetryForWorkflow(
    WorkflowTaskProposalDraft original,
    WorkflowTaskProposalDraft finalized,
    bool projectLooksEmpty,
  ...
  bool workflowPrefersExplicitSingleTask(
    ConversationWorkflowSpec workflowSpec,
  ) {
    return _workflowPrefersExplicitSingleTask(workflowSpec);
  }

  Set<String> explicitFirstSliceTargetFiles(
    ConversationWorkflowSpec workflowSpec,
  ) {
    return _explicitFirstSliceTargetFiles(workflowSpec);
  }

  List<ConversationWorkflowTask> reorderTaskProposalTasks(
    List<ConversationWorkflowTask> tasks, {
    required bool projectLooksEmpty,
  }) {
    return _reorderTaskProposalTasks(
      tasks,
      projectLooksEmpty: projectLooksEmpty,
    );
  }
