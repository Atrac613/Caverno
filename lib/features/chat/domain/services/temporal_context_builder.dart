class TemporalContextBuilder {
  ...
  static String? build({required DateTime now, required String userInput}) {
    ...
    return [
      ...
      '- this week (Mon-Sun) = ${_formatDate(thisWeekStart)} to ${_formatDate(thisWeekEnd)}',
      '- last week (Mon-Sun) = ${_formatDate(lastWeekStart)} to ${_formatDate(lastWeekEnd)}',
      '- next week (Mon-Sun) = ${_formatDate(nextWeekStart)} to ${_formatDate(nextWeekEnd)}',
      '- recent = ${_formatDate(recentStart)} to ${_formatDate(today)} (past 30 days)',
