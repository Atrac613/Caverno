...
class ToolTerminalResponsePolicy {
  ...
  /// Reads a final answer as claiming a background process finished.
  ///
  /// Triggering only: nothing here establishes that the job actually
  /// completed. It selects answers worth checking against the monitor,
  /// which is what owns that verdict.
  bool looksLikeBackgroundProcessCompletionClaim(String response) {
    final candidate = response.trim();
    if (candidate.isEmpty) {
      return false;
    }
