...
void _runCodingWorktreeSessionLauncher() {
  test(
    'creates composer worktrees under an assignment id path segment',
    () async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final calls = <_GitCallCodingWorktreeSessionLauncher>[];
...
CodingEnvironmentProcessRunner _gitRunner({
  required List<_GitCallCodingWorktreeSessionLauncher> calls,
  required String projectRoot,
}) {
  return (executable, arguments, {workingDirectory}) async {
    calls.add(
      _GitCallCodingWorktreeSessionLauncher(
        arguments: arguments,
    ...
    if (_argumentsEqualCodingWorktreeSessionLauncher(arguments, const [
      'rev-parse',
      '--show-toplevel',
    ])) {
    ...
    if (arguments.length == 6 &&
        _argumentsEqualCodingWorktreeSessionLauncher(
          arguments.take(4).toList(),
          const ['worktree', 'add', '-b', 'feature/fix-composer-flow-12345678'],
        ) &&
        arguments[4] ==
            '/Users/test/Workspace/caverno-worktrees/12345678/caverno' &&
        arguments[5] == 'main') {
...
bool _argumentsEqualCodingWorktreeSessionLauncher(
...
class _GitCallCodingWorktreeSessionLauncher {
  const _GitCallCodingWorktreeSessionLauncher({
