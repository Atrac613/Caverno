...
void main() {
  ...
  test(
    ...
    () async {
      ...
      // The loop asks once for the command the answer only described. This
      // model declines again, so the unverified notice is still the outcome —
      // but the notice is the fallback, not the first response.
      expect(dataSource.toolResultRequestCount, 1);
      expect(
        chatNotifier.state.messages.last.content,
  ...
  test(
    'sendMessage dispatches the retry call for a command an answer only '
    'described',
    () async {
      // Measured motivation: 25 of 130 turn exits in the session-log corpus
      // ended on the unverified notice with no recovery attempted, because the
      // older recovery is gated on the user typing "continue". Here the user
      // simply answers the assistant's proposal.
      final describedRun =
      ...
      // The retry's tool call is dispatched into the loop instead of the turn
      // ending on a notice. (A local command needs a coding project root to
      // reach execution, so the wiring is proven with an MCP tool here.)
