...
void main() {
  ...
  group('classify', () {
    test('an explicit loop hint always wins over derivation', () {
    ...
    test('a length finish reason is truncation, before content checks', () {
      expect(
        classifier.classify(state(text: 'The answer is', finishReason: 'length')),
        ToolLoopExitReason.lengthTruncated,
      );
    });

    test('empty content and the (empty) sentinel are emptyResponse', () {
      expect(classifier.classify(state(text: '')), ToolLoopExitReason.emptyResponse);
      expect(
    ...
    test('reaching the cap with pending tool calls is maxIterations', () {
      ...
      expect(
        classifier.classify(
          state(text: 'working on it', iteration: 10, maxIterations: 10, pending: true),
        ),
    ...
    test('a terse but terminated short answer stays a text response', () {
      expect(classifier.classify(state(text: 'Done.')), ToolLoopExitReason.textResponse);
      expect(classifier.classify(state(text: 'Yes!')), ToolLoopExitReason.textResponse);
    });
  });

  group('shouldExplain', () {
    test('explains empty and sentinel turns', () {
      expect(classifier.shouldExplain(ToolLoopExitReason.emptyResponse, ''), isTrue);
      expect(
    ...
    test('does not explain a healthy short answer', () {
      expect(classifier.shouldExplain(ToolLoopExitReason.textResponse, 'Done.'), isFalse);
    });

    test('explains a partial fragment but not a real reply on the same reason', () {
      expect(
      ...
      expect(
        ...
        isFalse,
        reason: 'a substantive reply is not overwritten just because the cap was hit',
      );
    });
  });
  ...
  group('completionExplanation', () {
    test('is null for a healthy text response (terse answers stay silent)', () {
      expect(classifier.completionExplanation(ToolLoopExitReason.textResponse), isNull);
    });

    test('is a non-empty sentence for every abnormal reason', () {
      for (final reason in ToolLoopExitReason.values) {
        if (reason == ToolLoopExitReason.textResponse) continue;
  ...
  group('logToken', () {
    test('maps every reason to a stable distinct snake_case token', () {
      final tokens =
          ToolLoopExitReason.values.map(classifier.logToken).toList();
      expect(tokens.toSet().length, tokens.length, reason: 'tokens are distinct');
      expect(classifier.logToken(ToolLoopExitReason.maxIterations), 'max_iterations');
      expect(
  ...
  group('isMidWorkStop', () {
    test('flags a turn whose last persisted message is a tool result', () {
      expect(
        classifier.isMidWorkStop(state(lastIsTool: true, lastTool: 'read_file')),
        isTrue,
      );
