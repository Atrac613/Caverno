...
import '../../tool/rag2_sqlite_durability_replay.dart';
...
void main() {
  test('reopens the last committed generation from a new connection', () async {
    ...
    expect(store.hasFts5Tables, isFalse);
    store.close();
    ...
    final generation = await reopened.read(_identity);
  ...
  test('rejects an unsupported schema without mutating rows', () async {
    final output = Directory.systemTemp.createTempSync('rag2-sqlite-schema-');
    addTearDown(() => output.deleteSync(recursive: true));
    final snapshots = await _snapshots();
    final path = '${output.path}/store.sqlite';
    final store = Rag2SqliteGenerationStore(
  ...
  test('isolates two projects in one SQLite file', () async {
    final output = Directory.systemTemp.createTempSync('rag2-sqlite-isolate-');
    addTearDown(() => output.deleteSync(recursive: true));
    ...
    final path = '${output.path}/store.sqlite';
    final first = Rag2SqliteGenerationStore(
      databasePath: path,
      projectId: 'persistence-project-a',
    );
    await first.apply(declarationIdentity: _identity, snapshot: firstSnapshot);
    first.close();
    final second = Rag2SqliteGenerationStore(
      databasePath: path,
      projectId: 'persistence-project-b',
    );
    await second.apply(
    ...
    final firstRead = Rag2SqliteGenerationStore(
      databasePath: path,
      projectId: 'persistence-project-a',
    );
    final secondRead = Rag2SqliteGenerationStore(
      databasePath: path,
      projectId: 'persistence-project-b',
    );
  ...
  test('rejects a snapshot that belongs to another project', () async {
    final output = Directory.systemTemp.createTempSync('rag2-sqlite-foreign-');
    addTearDown(() => output.deleteSync(recursive: true));
    final snapshots = await _snapshots();
    final store = Rag2SqliteGenerationStore(
      databasePath: '${output.path}/store.sqlite',
      projectId: 'persistence-project-a',
    );
    addTearDown(store.close);
  ...
  test('writes aggregate-only reports', () async {
    ...
    expect(report.toJson()['sqliteDurabilityDecision'], 'go');
    expect(report.toJson()['driftDecision'], 'not_selected');
