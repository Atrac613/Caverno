...
void _runRag2SemanticClaimEval() {
  ...
  test('fails closed when the semantic verifier is unavailable', () async {
    ...
    final report = await runRag2SemanticClaimEval(
      _optionsRag2SemanticClaimEval(directory.path),
  ...
  test('evaluates all fixed claims deterministically', () async {
    ...
    expect(
      File('${directory.path}/rag2_semantic_claim_eval.md').readAsStringSync(),
      report.toMarkdown(),
    );
  });
}

Rag2StructuredClaimOptions _optionsRag2SemanticClaimEval(String outDir) =>
