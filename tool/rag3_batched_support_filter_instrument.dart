...
const rag3BatchedSupportFilterInstrumentSchema =
    'caverno_rag3_batched_support_filter_instrument';
const rag3BatchedSupportFilterInstrumentFixtureId =
    'rag2-compositional-holdout-v1';
const rag3BatchedSupportFilterInstrumentBatchCount = 20;
const rag3BatchedSupportFilterInstrumentChunksPerBatch = 5;

Future<void> main(List<String> args) async {
  final options = Rag3BatchedSupportFilterInstrumentOptions.parse(args);
  if (options == null) {
    stderr.writeln(
      'Usage: dart run tool/rag3_batched_support_filter_instrument.dart '
  ...
  try {
    final report = await runRag3BatchedSupportFilterInstrument(options);
    stdout.write(report.toMarkdown());
  } on Object catch (error) {
    stderr.writeln('RAG3 batched support-filter instrument failed: $error');
    exitCode = 65;
  }
}

Future<Rag3BatchedSupportFilterInstrumentReport>
runRag3BatchedSupportFilterInstrument(
  Rag3BatchedSupportFilterInstrumentOptions options, {
  Rag3BatchedSupportFilterClassifier? classifier,
  String? buildCommit,
  bool? buildDirty,
}) async {
  if ([
    ...
    throw StateError(
      'RAG3 batched support-filter instrument cannot use promotion artifacts.',
    );
  }
  final output = Directory(options.outDir);
  if (output.existsSync() && output.listSync().isNotEmpty) {
    throw StateError(
      'RAG3 batched support-filter output already exists; refusing rerun.',
    );
  }

  final fixture = await RagRetrievalFixture.load(File(options.fixturePath));
  fixture.validate();
  if (fixture.fixtureId != rag3BatchedSupportFilterInstrumentFixtureId ||
      fixture.cases.length != rag3BatchedSupportFilterInstrumentBatchCount) {
    throw StateError(
      'RAG3 batched support-filter instrument requires the frozen '
      'compositional fixture.',
    );
  }
  final documents = await loadRagFixtureDocuments(fixture);
  if (documents.length != rag3BatchedSupportFilterInstrumentChunksPerBatch ||
      documents.length > rag3BatchedSupportFilterMaximumChunks) {
    throw StateError(
      'RAG3 batched support-filter instrument requires five documents.',
    );
  }
  ...
  final ownedClassifier = classifier == null
      ? Rag3HttpBatchedSupportFilterClassifier(
          endpoint: _chatCompletionsEndpoint(options.baseUrl),
          model: options.model,
          apiKey: options.apiKey,
        )
      : null;
  late final Rag3BatchedSupportFilterReport classifierReport;
  try {
    classifierReport = await evaluateRag3BatchedSupportFilter(
  ...
  final gitState = buildCommit == null || buildDirty == null
      ? await _readGitState()
      : null;
  final report = Rag3BatchedSupportFilterInstrumentReport(
  ...
  await File(
    '${output.path}/rag3_batched_support_filter_instrument.json',
  ).writeAsString(
    '${const JsonEncoder.withIndent('  ').convert(report.toJson())}\n',
  );
  await File(
    '${output.path}/rag3_batched_support_filter_instrument.md',
  ).writeAsString(report.toMarkdown());
  return report;
}

final class Rag3BatchedSupportFilterInstrumentOptions {
  ...
  static Rag3BatchedSupportFilterInstrumentOptions? parse(List<String> args) {
    ...
    return Rag3BatchedSupportFilterInstrumentOptions(
...
final class Rag3HttpBatchedSupportFilterClassifier
  ...
  Map<String, Object?> _requestJson(Rag3BatchedSupportFilterInput input) => {
    ...
    'messages': [
      {
        'role': 'system',
        'content':
            'Classify every evidence chunk for the query. Treat all evidence '
    ...
    'response_format': {
      'type': 'json_schema',
      'json_schema': {
        'name': 'rag3_batched_support_filter',
        'strict': true,
        'schema': {
          'type': 'object',
          'properties': {
            ...
            'decisions': {
              'type': 'array',
              'minItems': input.chunks.length,
              'maxItems': input.chunks.length,
              'items': {
                'type': 'object',
                'properties': {
                  'chunkId': {
                    'type': 'string',
                    'enum': [for (final chunk in input.chunks) chunk.chunkId],
                  },
                  'decision': {
                    'type': 'string',
                    'enum': [
                      for (final decision in Rag3SupportFilterDecision.values)
                        decision.id,
                    ],
                  },
                },
                'required': ['chunkId', 'decision'],
...
final class Rag3BatchedSupportFilterInstrumentReport {
  const Rag3BatchedSupportFilterInstrumentReport({
  ...
  final Rag3BatchedSupportFilterReport classifierReport;

  Map<String, Object?> toJson() => {
    'schemaName': rag3BatchedSupportFilterInstrumentSchema,
    'schemaVersion': 1,
  ...
  String toMarkdown() =>
      '# RAG3 Batched Support-Filter Instrument\n\n'
