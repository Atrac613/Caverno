...
Future<void> createRag2ChunkSearchTable(AppDatabase database) {
  return database.customStatement(
    ...
    'tokenize=$rag2Fts5SqliteTokenizer)',
  );
}
...
Future<void> replaceRag2ChunkSearchIndex(
  ...
  var settled = false;
  await database.customStatement('BEGIN IMMEDIATE');
  try {
    await database.customStatement(
      'DELETE FROM $rag2ChunkSearchTable '
      'WHERE project_identity = ? AND declaration_identity = ?',
      [target.projectIdentity, target.declarationIdentity],
    );
    for (final chunk in chunks) {
      final terms = tokenizeRag2Lexical(
        chunk.content,
    ...
    beforeCommit?.call();
    await database.customStatement('COMMIT');
    settled = true;
  } on Object {
    if (!settled) {
      try {
        await database.customStatement('ROLLBACK');
      } on Object {
