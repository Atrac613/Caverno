...
IOS_PROVISIONING_PROFILE="${CAVERNO_IOS_PROVISIONING_PROFILE:-Caverno AppStore Provisioning Profile}"
IOS_WATCH_APP_BUNDLE_ID="${CAVERNO_IOS_WATCH_APP_BUNDLE_ID:-com.noguwo.apps.caverno.watchkitapp}"
IOS_WATCH_APP_PROVISIONING_PROFILE="${CAVERNO_IOS_WATCH_APP_PROVISIONING_PROFILE:-Caverno WatchApp App Store Profile}"
IOS_WATCH_WIDGET_BUNDLE_ID="${CAVERNO_IOS_WATCH_WIDGET_BUNDLE_ID:-com.noguwo.apps.caverno.watchkitapp.widget}"
IOS_WATCH_WIDGET_PROVISIONING_PROFILE="${CAVERNO_IOS_WATCH_WIDGET_PROVISIONING_PROFILE:-Caverno WatchApp Widget App Store Profile}"

MACOS_NOTARY_PROFILE="${CAVERNO_NOTARYTOOL_PROFILE:-caverno-notary}"
MACOS_PACKAGE="${CAVERNO_MACOS_SPARKLE_PACKAGE_FORMAT:-zip}"
MACOS_DOWNLOAD_URL_PREFIX="${CAVERNO_SPARKLE_DOWNLOAD_URL_PREFIX:-https://d1ap7clvx8zf86.cloudfront.net/caverno/macos}"
...
$(if [[ "${IOS_SIGNING_STYLE}" == "manual" ]]; then cat <<PROFILE_PLIST
  ...
  <dict>
    ...
    <key>${IOS_WATCH_APP_BUNDLE_ID}</key>
    <string>${IOS_WATCH_APP_PROVISIONING_PROFILE}</string>
    <key>${IOS_WATCH_WIDGET_BUNDLE_ID}</key>
    <string>${IOS_WATCH_WIDGET_PROVISIONING_PROFILE}</string>
  </dict>
PROFILE_PLIST
fi)
  <key>teamID</key>
...
release_ios() {
  ...
  # Conditional lane execution disables errexit inside this function.
  caverno_load_dart_define_args "${ROOT_DIR}" require || return $?
