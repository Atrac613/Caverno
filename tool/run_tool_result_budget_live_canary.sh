...
REPORT_ROOT="${CAVERNO_TOOL_RESULT_BUDGET_LIVE_CANARY_REPORT_ROOT:-${CAVERNO_LIVE_LLM_CANARY_REPORT_ROOT:-${ROOT_DIR}/build/integration_test_reports}}"
RUN_DIR="${REPORT_ROOT}/tool_result_budget_live_canary_$(date +%s)"
LOG_PATH="${RUN_DIR}/flutter_test.jsonl"
REPORTER="json"

echo "Running tool-result budget live LLM canary"
...
flutter test tool/canaries/tool_result_budget_live_llm_canary_test.dart -r "${REPORTER}" > "${LOG_PATH}" 2>&1
TEST_STATUS=$?
set -e
...
dart run "${ROOT_DIR}/tool/live_llm_canary_summary.dart" \
  --log "${LOG_PATH}" \
  --out-dir "${RUN_DIR}" \
  --canary-name tool_result_budget_live_canary \
  --surface chat_budget \
  --base-url "${CAVERNO_LLM_BASE_URL}" \
  --model "${CAVERNO_LLM_MODEL}" \
  --command "tool/run_tool_result_budget_live_canary.sh"
SUMMARY_STATUS=$?
set -e

if [ "${TEST_STATUS}" -ne 0 ]; then
  echo "Tool-result budget live LLM canary failed. Flutter JSON log: ${LOG_PATH}"
  exit "${TEST_STATUS}"
fi
