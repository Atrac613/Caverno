...
Future<void> main(List<String> args) async {
  ...
  for (var index = 0; index < args.length; index += 1) {
    ...
    switch (arg) {
      ...
      case '--m39-beta-signoff':
        index += 1;
        if (index >= args.length) {
          return _usageError('--m39-beta-signoff requires a value.');
        }
        m39BetaSignoffPath = args[index];
      case '--diagnostics':
        index += 1;
        if (index >= args.length) {
  ...
  if (writeTemplateOnly) {
    final templateFile = File(
      templatePath ??
          '${reportRoot.path}/macos_computer_use_m40_launch_checklist_template.json',
    );
    ...
    stdout.writeln(
      'M40 launch checklist template written to ${templateFile.path}',
    );
    return;
  }
  ...
  final inputs = readMacosComputerUseProductionLaunchInputs(
    ...
    m39BetaSignoffPath: m39BetaSignoffPath,
    diagnosticsPath: diagnosticsPath,
  );
  final summary = buildMacosComputerUseProductionLaunchSummary(inputs);
  final outputJson = File(
    outputJsonPath ??
        '${reportRoot.path}/macos_computer_use_m40_production_launch_gate.json',
  );
  final outputMarkdown = File(
    outputMarkdownPath ??
        '${reportRoot.path}/macos_computer_use_m40_production_launch_gate.md',
  );
  ...
  stdout.writeln('M40 production launch gate written to ${outputJson.path}');
...
void _printUsage() {
  stdout.writeln(
    'Usage: dart run tool/macos_computer_use_production_launch_gate.dart '
    '[--root path] [--launch-checklist path] '
    '[--release-artifact-report path] [--release-packaging-report path] '
    '[--manual-tcc-report path] [--m36-live-llm-eval path] '
    '[--m39-beta-signoff path] [--diagnostics path] '
...
bool _shouldExitFailure(
  MacosComputerUseProductionLaunchSummary summary,
  String exitPolicy,
) {
