...
: "${CAVERNO_LLM_BASE_URL:?Set CAVERNO_LLM_BASE_URL before running the pending-action length recovery live canary.}"
: "${CAVERNO_LLM_API_KEY:?Set CAVERNO_LLM_API_KEY before running the pending-action length recovery live canary.}"
: "${CAVERNO_LLM_MODEL:?Set CAVERNO_LLM_MODEL before running the pending-action length recovery live canary.}"

REPORT_ROOT="${CAVERNO_CODING_PENDING_ACTION_LENGTH_RECOVERY_REPORT_ROOT:-${CAVERNO_LIVE_LLM_CANARY_REPORT_ROOT:-${ROOT_DIR}/build/integration_test_reports}}"
RUN_DIR="${REPORT_ROOT}/coding_pending_action_length_recovery_live_canary_$(date +%s)"
...
echo "Running coding pending-action length recovery live canary"
echo "  Injection: one final tool-result stream with finishReason=length"
...
set +e
CAVERNO_CODING_PENDING_ACTION_LENGTH_RECOVERY_LIVE_CANARY=1 \
...
"${DART_CMD[@]}" run "${ROOT_DIR}/tool/live_llm_canary_summary.dart" \
  ...
  --command "tool/run_coding_pending_action_length_recovery_live_canary.sh"
SUMMARY_STATUS=$?
set -e

if [ "${TEST_STATUS}" -ne 0 ]; then
  echo "Coding pending-action length recovery live canary failed."
  echo "  Flutter JSON log: ${LOG_PATH}"
  echo "  Session logs: ${SESSION_LOG_ROOT}"
  exit "${TEST_STATUS}"
fi

echo "Coding pending-action length recovery live canary passed."
echo "  Flutter JSON log: ${LOG_PATH}"
echo "  Session logs: ${SESSION_LOG_ROOT}"
exit "${SUMMARY_STATUS}"
