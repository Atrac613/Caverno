...
: "${CAVERNO_LLM_BASE_URL:?Set CAVERNO_LLM_BASE_URL before running the Markdown TOC live canary.}"
: "${CAVERNO_LLM_API_KEY:?Set CAVERNO_LLM_API_KEY before running the Markdown TOC live canary.}"
: "${CAVERNO_LLM_MODEL:?Set CAVERNO_LLM_MODEL before running the Markdown TOC live canary.}"

REPORT_ROOT="${CAVERNO_CODING_MARKDOWN_TOC_REPORT_ROOT:-${CAVERNO_LIVE_LLM_CANARY_REPORT_ROOT:-${ROOT_DIR}/build/integration_test_reports}}"
RUN_DIR="${REPORT_ROOT}/coding_markdown_toc_live_canary_$(date +%s)"
...
echo "Running Coding Markdown TOC short-prompt live canary"
echo "  Language: Dart"
...
set +e
CAVERNO_CODING_MARKDOWN_TOC_LIVE_CANARY=1 \
...
"${DART_CMD[@]}" run "${ROOT_DIR}/tool/live_llm_canary_summary.dart" \
  --log "${LOG_PATH}" \
  --out-dir "${RUN_DIR}" \
  --canary-name coding_markdown_toc_live_canary \
  --surface coding_mvp \
  --base-url "${CAVERNO_LLM_BASE_URL}" \
  --model "${CAVERNO_LLM_MODEL}" \
  --command "tool/run_coding_markdown_toc_live_canary.sh"
SUMMARY_STATUS=$?
set -e

if [ "${TEST_STATUS}" -ne 0 ]; then
  echo "Coding Markdown TOC live canary failed."
  echo "  Flutter JSON log: ${LOG_PATH}"
  echo "  Session logs: ${SESSION_LOG_ROOT}"
  exit "${TEST_STATUS}"
fi

echo "Coding Markdown TOC live canary passed."
echo "  Flutter JSON log: ${LOG_PATH}"
echo "  Session logs: ${SESSION_LOG_ROOT}"
exit "${SUMMARY_STATUS}"
