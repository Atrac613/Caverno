...
: "${CAVERNO_LLM_BASE_URL:?Set CAVERNO_LLM_BASE_URL before running the production-path TODO canary.}"
: "${CAVERNO_LLM_API_KEY:?Set CAVERNO_LLM_API_KEY before running the production-path TODO canary.}"
: "${CAVERNO_LLM_MODEL:?Set CAVERNO_LLM_MODEL before running the production-path TODO canary.}"
...
SUITE_REPORT="${PLAN_REPORT_ROOT}/plan_mode_live_suite_macos_report.json"
...
if not report_path.is_file():
    print(f"Missing TODO canary suite report: {report_path}", file=sys.stderr)
    sys.exit(1)
...
if quality.get("ready") is not True or blocker_count != 0:
    print(
        f"TODO canary report quality is blocked: {blocker_count} blocker(s)",
        file=sys.stderr,
    )
    sys.exit(1)
PY

echo "Production-path TODO app Live canary passed."
echo "  Report directory: ${RUN_DIR}"
