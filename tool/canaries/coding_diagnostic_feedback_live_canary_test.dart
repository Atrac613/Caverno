...
''',
  ...
  Future<ChatCompletionResult> createChatCompletionWithToolResults({
    ...
    _printAnalyzerFeedbackSummaries(toolResults);
  ...
  void _printAnalyzerFeedbackSummaries(List<ToolResultInfo> toolResults) {
    for (final result in toolResults) {
      if (result.name != CodingDiagnosticFeedbackService.toolName) {
        continue;
      }
      final payload = _tryDecodeObject(result.result);
      final rawChangedPaths = payload['changed_paths'];
      final changedPaths = rawChangedPaths is Iterable
          ? rawChangedPaths
                .whereType<String>()
                .where((path) => path.trim().isNotEmpty)
                .toList(growable: false)
          : const <String>[];
      final rawDiagnosticCount = payload['diagnostic_count'];
      final diagnosticCount = rawDiagnosticCount is num
          ? rawDiagnosticCount.toInt()
          : 0;
      debugPrint(
        '[CodingDiagnostics] Analyzer feedback summary: '
        '${jsonEncode({'toolName': CodingDiagnosticFeedbackService.toolName, 'diagnosticCount': diagnosticCount, 'files': changedPaths})}',
      );
    }
  }
}
