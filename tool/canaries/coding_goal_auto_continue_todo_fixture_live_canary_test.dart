...
/// The source of the verifier the model is told to run.
///
/// This file used to be `void main() {}`, on the belief — stated in its own
/// comment — that the harness intercepted the command. Nothing does:
/// `local_execute_command` is a built-in tool, so the fixture's same-named MCP
/// definition is shadowed and the model really executes this file. A
/// placeholder therefore answered every verification with a silent exit 0, and
/// a model that ran exactly what it was told to run was handed a green it had
/// no way to doubt.
///
...
Directory _projectRoot() {
  ...
  if (File('\${fromScript.path}/pubspec.yaml').existsSync()) {
    return fromScript;
  }
...
Future<void> _runTodoMvpLiveScenario(
  ...
  try {
    ...
    // Read from the verifier's own log rather than from toolService: this
    // scenario runs local_execute_command through Caverno's built-in handler,
    // so the fixture's same-named MCP tool is shadowed and never records the
    // run. Counting there measured nothing, and reported a red the model had
    // no part in.
    final verifierRuns = _recordedVerifierRuns(fixture.root);
    expect(
      verifierRuns,
      isNotEmpty,
      reason: '$diagnostic\nThe model never ran "$_verifyCommand".',
    );
    expect(
      verifierRuns.any((run) => run['exit_code'] == 0),
      isTrue,
    ...
    // The two checks below run against the fixture service, which this
    // scenario bypasses, so they hold vacuously here. They stay for the
    // MCP-routed scenarios that share this body.
    expect(
      _unchangedPathBackedVerifierReplays(toolService.executedCalls),
      isEmpty,
      reason:
...
bool _hasTerminalVerifierSuccess(List<Map<String, dynamic>> entries) {
  return entries.any(_containsTerminalVerifierSuccess);
}
