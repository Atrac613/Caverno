...
#
# Honors CAVERNO_HOME (default ~/.caverno).
set -euo pipefail
...
python3 - "$HOME_DIR" "$N" "$AUDIT_DATE" <<'PY'
import json, glob, os, sys, datetime

home, n, audit_date = sys.argv[1], int(sys.argv[2]), sys.argv[3]

logs = sorted(
    glob.glob(f"{home}/session_logs/*/*.jsonl"),
...
af = f"{home}/approval_audit/{audit_date}.jsonl"
