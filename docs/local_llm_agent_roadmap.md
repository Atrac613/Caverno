...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. Manifest-shadow, metadata-only source-scope, three-command batch Git inventory, and their integration are Go, but Caverno remains No-Go: runtime-only exceeds the default file ceiling, and retaining runtime, tests, and top-level docs exceeds the hard ceiling. Compare answer-bearing coverage across the measured source-role profiles next; do not select a scope, raise limits, or start storage. |
