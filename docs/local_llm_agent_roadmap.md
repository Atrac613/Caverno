...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. The opt-in manifest-shadow v1 boundary is Go, but the Caverno preflight is No-Go at 2,816 candidates versus the 2,048 hard ceiling. Measure top-level source scopes next; do not raise the cap or start storage. |
