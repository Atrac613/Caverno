...
- `KC<number>` — Knowledge Currency: closing the gap between the model's
  training cutoff and the user's present environment, by pushing measured
  ground truth into the prompt instead of asking a stale model to suspect
  itself.
...
| Retrieval | RAG3 | blocked | M | RAG2, LL5, F6, LL39 | The frozen one-shot candidate is No-Go. The verbose two-bucket classifier is quality Go but inline latency No-Go at 6,464 ms p95. The compact v3 candidate cut p95 to 1,614 ms but is also No-Go: it exceeded the 1,200 ms ceiling and produced 3 false positives plus 1 false negative. Prior deterministic score, intent, and cross-arm policies remain rejected. Bounded vector persistence, `search_knowledge`, prompting, promotion, and runtime wiring remain blocked. Evidence: `docs/rag3_compact_support_filter_instrument_2026-09-01.md`. |
...
Full design: `docs/knowledge_currency_track_design.md`.

Thesis 6 names the problem — local models are stale and change weekly — and LL10
answered one half of it. This track states the other half precisely.

The cutoff problem is not missing knowledge. A model that knows it is missing
something already behaves correctly: it searches, or it hedges. The damaging
case is the inverse — **the model does not know which of its beliefs expired**,
and answers with the fluency of a fact it learned ten thousand times in
training, true in 2024. Every freshness mechanism in the app today is **pull**:
`search_web`, `web_url_read`, and `resolve_installed_dependency` all require the
model to suspect staleness first. Suspicion is exactly the faculty a stale model
lacks, and `knowledgeCutoffHumilityInstruction`
(`system_prompt_constants.dart:30`) asks it to make precisely that judgment.

Track rule: **push what is cheap and certain, pull only what is expensive, and
never ask the model to decide whether it is stale.** The same trigger/judge
split the roadmap applies elsewhere — heuristics nominate, ground truth decides.
...
corollary against a third knowledge store applies. Automatic retrieval stays
behind RAG5's shadow gates. Web-fetched freshness data remains external evidence
under SEC1/SEC2 and never acquires instruction authority.

Ordering constraint (the only hard one): **capture the KC1 baseline before KC2
lands.** KC2 changes the prompt, and once it ships the before/after comparison
is gone.
...
- Tail placement is load-bearing: the block changes per project and per lockfile
...
"flutter_riverpod 3.1.2 is what is installed" is a fact the model can write code
against.
...
invented or one that exists only upstream — the failure its canary measured and
closed. It cannot detect a **deprecated-but-still-present** API, which is the
shape most real version drift takes: the old idiom still resolves, compiles with
a deprecation warning, or resolves and behaves differently. LL10 answers
...
itself a drift source.

Not a RAG milestone: no index, no ranking, no embeddings — a path lookup keyed
by the lockfile, so it must not queue behind RAG1-RAG3.
...
Anti-goals: no confidence scoring of the model's prose, and no verdict rendered
by the regex.
...
Recording `unknown` honestly is preferable to a probed number nobody trusts.

Deferred:
- Until a second model family is in regular production use, a static table for
  the one endpoint in use is not worth the schema change.
...
`trigram_or_idf` candidate at threshold `0.15` through an actual
file-backed AppDatabase generation and `rag2_chunk_search` slot. It
retains 16/16 answerable cases and 4/4 Japanese cases with MRR@5
`0.896`, but returns evidence for 2/4 no-answer cases against the
predeclared maximum of 1/4. Provenance validation, the empty negative
control, schema version 5, conversation search, and the seeded LL5
embedding row pass. The evaluation contract is Go; the candidate,
...
withdraws raw no-answer count as the RAG2 promotion question. The contract,
20-case holdout, and evaluator were committed in that order. The unchanged
hosted candidate passes with 14/14 answer support, 4/4 Japanese support, 2/2
expected abstention support, zero unavailable cases with only irrelevant
evidence, support MRR@5 `1.000`, nDCG@5 `0.995`, and 3,776/6,000 context
tokens. Raw no-answer retrieval remains visible at 3/4: two cases carry
abstention support, one carries topical-only evidence, and one returns no
...
- **LL34 and LL35 rest on observation.** Four consumers re-parsing the same
  string is visible in the code; `_looksComplete` containing `'passed'` is
  visible in the code. Neither is a hypothesis about behavior, so neither needs
  a triage gate.
- **LL36 and LL37 rest on hypotheses.** "The lexical guards may be dead weight"
  and "there is a residue ground truth cannot decide" are both unmeasured. They
  get the same discipline that demoted LL29.

That makes **LL36 the instrument for LL37, exactly as LL31 was for LL29/LL30**:
the firing distribution it produces is what says whether a verification stage
has anything left to decide. LL35 could demote its lexical terminal transitions
without stranding weak models because the per-model confirmation policy landed
with the demotion. LL37 is still gated on LL36's **measurements**, not merely on
LL36 having shipped.

1. **LL34** restores structure at the tool boundary — deletes regex rather than
   adding heuristics, and its highest-value field feeds the dominant measured
   failure.
2. **LL35** makes the model state completion explicitly, reconciles the final
   turn evidence, and supplies a user-confirmation fallback before removing
   lexical prose from terminal authority.
3. **LL36** instruments every remaining lexical guard and produces the residue
   measurement that decides LL37.
4. **LL37** is the expensive one, deliberately last, and may well not be built.

Removal protocol — **shadow mode, always.** A replacement runs beside the
incumbent with both verdicts recorded and disagreements logged. LL35 keeps the
lexical parser as a shadow-only comparator while tool, structured task, and
user-confirmed signals own terminal state; its standalone marker is required
because the comparison is known after the immutable turn-exit snapshot. Other
lexical guards still require LL36 firing evidence before deletion. This keeps
deletion measurable, following the same evidence-first discipline as the
LL29-LL31 track.
...
eight. The implementation surface is small *because* usage is concentrated — the
...
- Add typed outcome fields to the tool-result envelope alongside the existing
  ...
  - `exitCode` for command-execution tools (27.4% of traffic),
  - `fileMutations` (path, content hash, byte size) for filesystem writes
    (13.9%),
  - `readOutcome` (path, content hash, bytes, lines) for file reads — see below,
  - `testOutcome` (passed / failed / skipped counts, command) where a
    recognized test runner produced the output (1.2%).
- **The read hash is the highest-value field, which the census only made
  visible.** `read_file` is the single most-invoked tool (26.3%) and was left
  out of the original scope because a read has no pass/fail outcome. It has a
  better one: the content hash. Today `read_file` returns `content` and
  `truncated` only (`filesystem_tools.dart:242`). A hash makes the project's
  *measured dominant failure* — redundant re-reads in ~53% of sessions per the
  LL31 triage — a field comparison instead of a heuristic:
  `ToolLoopContextDigest` currently infers `unchanged` by comparing whole
  rendered payloads for byte-identity, and with a hash the harness can state
  "this file is unchanged since iteration 4" as a fact. This is what makes LL34
  a contributor to the top real failure mode rather than pure hygiene, and it
  is the strongest argument for building it ahead of the track's speculative
  half.
...
**Shadow mode and its denominator are implemented and remain active after the
authority change.** Commit `e77a57e7` records one comparison for every eligible
turn that started with an active goal, including agreements, while retaining
the three stable disagreement labels. `tool/triage_session_logs.py` aggregates
the agreement distribution and treats old disagreement-only markers
compatibly. Lexical completion now feeds that comparator only; it cannot close
or block a goal.
...
Historical decision before the completion slice: the **LL3 fidelity probe,
confirmation rung, and lexical authority change were held** because three
sessions of one fixture were not broad enough to establish coverage. The user
then explicitly requested all remaining LL35 implementation. The shipped path
does not turn that narrow sample into stronger evidence: it makes the authority
change safe through an explicit per-model policy and user-confirmation fallback
while retaining the lexical comparison marker.

**Completion update, 2026-08-09.** The bounded next-step selector, LL3
`update_goal_fidelity` probe and stored profile verdict, per-model
`tool` / `tool_or_ask` / `ask` setting, budget/no-work confirmation path,
confirmation summary/actions, blocker persistence, paused-at-cap handling, and
final-turn acknowledgement reconciliation are implemented. Lexical completion
and blocker prose remain measurable but cannot set `completed` or `blocked`;
all-completed saved workflow tasks remain a structured completion signal.

A direct non-executing probe against `qwen3.6-27b-vision` at the configured LAN
endpoint returned exactly one `update_goal` call with
`{"completed":true}` and no text, so the observed model verdict is `reliable`.
A distinct Markdown TOC goal produced two additional owner-scoped comparisons,
both `agree`, with zero unknowns or disagreements. The overall canary correctly
failed because a repair after failed verification was not revalidated; it is
recorded as a safety-gate failure, not as a successful task run. Its first
hidden continuation also showed the selected immediate plan step. The pooled
post-denominator sample is therefore 15 comparisons: 12 agree, 3 explained
`goal_completion_tool_accepted_lexical_missed`, and 0 unknown.
...
Progress (completed on branch `feat/ll35-shadow-denominator`):
...
- **Slice 2 (`0b501c08`)** — `update_goal` registered and dispatched through the
  ...
  is a tool failure. At this staging slice lexical inference still remained
  authoritative.
- **Slice 3 (`97cb16b6`)** — `GoalCompletionShadow` records where the tool and
  the lexical path decided completion differently. This initial
  disagreement-only turn-exit representation was superseded by Slice 4's
  denominator-aware standalone marker, because the final comparison is known
  only after goal-turn persistence.
...
- **Completion slice (2026-08-09)** — final owner evidence now decides the tool
  acknowledgement; exact blockers persist as `blocked`; budget-cap progress
  pauses for confirmation; and confirmation-required tool claims become an
  owner-scoped pending decision. The bounded selector reads at most 8 KiB,
  prefers the typed active task, then reads the first unchecked item under the
  exact `## Task checklist` section, drops partial trailing input, and preserves
  the generic fallback. Model diagnostics record `update_goal` fidelity without
...
1. **Idle only — there is no inline stage.** The first pass of this design kept
   a single cheap verifier on the user's turn. The LL35 confirmation rung
   removes its justification: while a user is present, "here is what I did and
   what I could not verify — is this done?" is both more accurate and cheaper
   than any local verifier. What remains is the case where **no user is
   present** — unattended overnight runs, routines, LL13 worktree agents — and
   that is exactly the idle budget (Thesis §4, §7: tokens are free locally and
   idle hardware is a scheduled compute budget). So the panel runs through the
   LL18 orchestrator as the objective-level sibling of LL7 retry-until-green,
   and nothing is added to the user's turn (Thesis §3: prefill latency is the
   local UX killer). An overnight report — "three of these five goals do not
   actually meet their objective" — is a better product than 40 seconds added
   to every completion.
...
Scope:
- **Idle panel only.** N verifiers with majority-refute aggregation, scheduled
  by LL18 over the LL8 mesh using LL20 slots, run against goals completed by
  unattended paths (routines, overnight retry-until-green, LL13 worktree
  agents). Each is given the objective, the plan, the turn's changed-file set
  (LL34), and the implementer's captured evidence, and returns the fixed-schema
  JSON verdict — `refuted`, `findings[]` (kind / location / detail),
  `evidence`, `confidence`, `blocking`. Results land in a verdict store the
  user reviews, never as a blocking gate on an interactive turn.
- **Nothing on the user's turn.** An attended completion resolves through LL35:
  the tool call if the model is capable of it, the confirmation rung otherwise.
...
- No interactive turn gains measurable latency from this milestone (the panel is
  unreachable from the attended path — enforced by test, not convention).
- A model below the verifier fidelity threshold disables the panel entirely
  rather than emitting low-confidence refutes (test over a profile that fails
  the gate).
- Token cost per verified goal is recorded and bounded.

Gated on **LL36's measurements**, not merely on LL34-LL36 having shipped. LL36's
firing distribution is what says whether a residue exists that ground truth and
structured self-report cannot decide. This is the same instrument-first
discipline that LL31 imposed on LL29/LL30 — and LL29's demotion is the reminder
that the gate is real.

Harvest note — **the convergence controls are worth taking even if this
milestone never ships.** Anti-ratchet, stall exit, and blocking classification
are prompt/loop discipline that applies to `RetryUntilGreenCoordinator` today,
where a repeatedly-failing verification loop has the same non-convergence
failure mode. Extracting them into the existing LL7 loop is a small, independent
slice with none of this milestone's cost or risk, and it should not wait on the
panel. If the LL36 measurement kills LL37, that harvest is what survives.
...
- `tool/ll37_routine_history_export.dart` now converts one explicitly consented
  ...
  After explicit approval, the real pair was scored live on
...
3. **SEC4.3 — Network authority and resource boundary.** Land as focused
   ...
   **SEC4.3e (P0 follow-up, completed 2026-08-24)** restricts HTML Preview to
   ...
   JSON document, and aggregate tool content consumption; **SEC4.3f-C
   (completed 2026-08-24)** rejects oversized Base64 and compressed settings QR
   input and caps chunked gzip output before UTF-8 or JSON decoding.
