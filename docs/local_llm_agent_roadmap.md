...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. Complete caller-declared source roots are promotion Go. Frozen-declaration acquisition now reruns Git-backed attestation in CI. A backend-neutral atomic generation replay passes apply, no-op, replacement, and rollback gates. Storage semantics are Go; retrieval is not evaluated and no backend or production path is selected. |
...
pins. This does not unify acquisition/storage declaration hashes or select a
backend. Evidence:
`docs/rag2_explicit_source_roots_acquisition_ci_2026-08-26.md`.
