...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. Stratified stable-hash sampling is closed after 6/8 development and 4/8 holdout coverage. The complete caller-declared source-roots contract is Go; a five-root chat memory/persistence development declaration passes acquisition preflight with 451 sources. Question evaluation, source selection, and storage remain No-Go. |
...
eligible sources with zero blockers. No new evaluation questions exist yet;
next create an in-scope/out-of-scope development set without changing the
roots. Storage, retrieval, settings, and application wiring remain out of
scope. Evidence:
