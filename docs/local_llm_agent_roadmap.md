...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. Acquisition and source-role oracle coverage contracts are Go, but source selection remains No-Go: the three measured profiles cover only 2/8, 4/8, and 6/8 fixed questions; the largest also exceeds the hard ceiling. Define one structural bounded profile candidate, then apply it unchanged to an untouched holdout; do not raise limits or start storage. |
...
validated evidence in runtime, documentation, test, tooling, and root-source
roles. The all-candidates control covers 8/8 but exceeds the hard ceiling. The
three measured profiles cover 2/8, 4/8, and 6/8; the largest has 2,562 files,
still omits tooling and root-source evidence, and exceeds the hard ceiling.
This is path-membership coverage, not retrieval evidence. Freeze it as a
development fixture. Next define one question-independent bounded profile and
apply it unchanged to a separate untouched holdout before selecting a scope.
