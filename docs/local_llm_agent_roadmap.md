...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. Stratified stable-hash sampling is closed after 6/8 development and 4/8 holdout coverage. The next frozen hypothesis admits complete caller-declared source roots under unchanged default limits; implementation, evaluation, source selection, and storage remain No-Go. |
...
The next slice may implement only an opt-in synthetic replay; active-project
fixtures, storage, retrieval, settings, and application wiring remain out of
scope. Evidence:
