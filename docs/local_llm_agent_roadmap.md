...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. Complete caller-declared source roots are promotion Go. A backend-neutral atomic generation replay now passes initial apply, identical no-op, stale replacement, and rollback gates. Storage semantics are Go; retrieval is not evaluated and no backend or production path is selected. |
