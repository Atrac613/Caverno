...
| Retrieval | RAG2 | later | M | RAG1, F4, LL4, SEC1 | Provenance-bearing Knowledge Objects and an incremental SQLite/FTS5 index over active-project code and Markdown. Acquisition and source-role required-source coverage v2 are Go, but source selection remains No-Go: the three measured profiles cover only 2/8, 4/8, and 6/8 development questions, and every profile exceeds the default ceiling. Define one structural bounded profile candidate, then apply it unchanged to an untouched holdout; do not raise limits or start storage. |
