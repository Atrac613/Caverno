...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. The frozen structural profile fits the default ceiling at 509 files but covers only 6/8 informed development questions. Apply it unchanged to one untouched holdout, then decide whether to close stratified sampling; do not tune quotas, raise limits, or start storage. |
