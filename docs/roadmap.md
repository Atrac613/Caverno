...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. Required-source coverage contract v2 is Go after bounded-read and eligibility repairs, but all measured profiles remain No-Go and exceed the default ceiling. Define one structural bounded profile, then apply it unchanged to an untouched holdout; do not raise limits or start storage. |
