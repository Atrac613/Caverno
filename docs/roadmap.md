...
| Remote Coding | RC1 | next | Add authenticated confidential transport, downgrade rejection, bounded unauthenticated connections/frames, reconnect resilience, support diagnostics, and multi-device evidence. | SEC4.5e connection admission, authentication deadlines, frame-size limits, and phase-aware message-rate limits are complete. Continue with reconnect resilience and product-promotion evidence. |
...
| Retrieval | RAG1 | next | Establish a versioned retrieval, answer-grounding, latency, resource, and token evaluation contract before production RAG work. | Start with a deterministic 20-case seed that distinguishes current source facts, historical agent-kb decisions, conflicts, Japanese queries, and unanswerable cases. |
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | Promote only after RAG1; preserve LL5, enforce project containment, and ship lexical-only correctness before embeddings. |
| Retrieval | RAG3 | later | Add bounded vector retrieval, weighted RRF, context budgeting, and an active-project `search_knowledge` tool. | Reuse LL5/LL39 embedding paths, remain lexical-capable when embeddings fail, and require the RAG1 quality/resource gates. |
...
| Knowledge Currency | KC1 | next | Measure claim correctness, not only tool coverage: classify version-sensitive prose and code-artifact claims, compare asserted values with a fixture oracle, and record separate truth (`correct` / `stale` / `unscorable`) and grounding (`supported` / `contradicted` / `absent`) verdicts plus prompt/tool/none provenance. | Instrument before mechanism, per LL31/LL36. Use fixed paired replays and per-class stale/unsupported rates plus detector precision/recall. Correct and stale claims with absent grounding must retain different truth verdicts. |
| Knowledge Currency | KC2 | next | Push measured toolchain and dependency ground truth into the prompt while preserving the datetime anchor that `SystemPromptBuilder` already emits unconditionally. | Not gated on KC1, but freeze the KC1 baseline first. Extract a shared LL10 dependency inventory, attest locked versus installed versions as exact/mismatch/unverifiable, omit non-exact versions from authoritative context, cache by project/metadata fingerprints, and keep the block in the dynamic tail. |
| Knowledge Currency | KC3 | later | Extend LL10 with installed version-delta evidence: bounded CHANGELOG/migration sections and declared deprecations from the attested local package source. | Close the deprecated-but-still-present blind spot without a second resolver or knowledge store. Add a new public tool name only if tool-discovery evaluation rejects an LL10 query mode; preserve containment, provenance, and response budgets. |
| Knowledge Currency | KC4 | later | Nominate cutoff-sensitive claims from visible prose, response code blocks, changed dependency-using code, and LL11 deprecation diagnostics; let only ground-truth evidence render the verdict. | Reuse existing recovery plumbing with a bounded turn-evidence adapter for artifacts. Annotate rather than block when verification is unavailable. Shadow precision and recall must include a stale API that appears only in edited code. |
...
| Security | SEC4 | current | Close the runtime trust, egress, transport, and local-data findings recorded in the 2026-08-14 audit and 2026-08-24 follow-up. | Start SEC4.3e HTML Preview active-content containment. SEC4.4g opaque local-command authority completed on 2026-08-24; resume remaining SA-16 work after SA-20 unless HTML Preview is mechanically disabled. |
...
| 2 | SEC4.3e | next | SA-20 | Constrain HTML Preview file exposure and active-content egress. | Declared preview assets load while fetch, beacon, form, frame, WebSocket, and external subresource exfiltration tests fail closed. |
| 3 | SEC4.3f | later | SA-21 | Apply bounded streaming and decompression limits to application-owned MCP and QR inputs. | Oversized HTTP, stdio, JSON-document, and gzip-expansion fixtures fail before unbounded buffering or decoding. |
| 4 | SEC4.6k | later | SA-22 | Enforce owner-only sensitive log storage and structured diagnostic redaction. | New, existing, and rotated logs use owner-only modes; nested and serialized credentials are redacted. |
| 5 | SEC4.5g / RC1 | later | SA-23 | Recheck pending-interaction origin and document or enforce device ownership at resolution. | Desktop-origin, stale, revoked-device, and cross-device authorization cases follow the declared policy. |
