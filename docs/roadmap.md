...
| Remote Coding | RC1 | next | Add authenticated confidential transport, downgrade rejection, bounded unauthenticated connections/frames, reconnect resilience, support diagnostics, and multi-device evidence. | SEC4.5c pinned WSS and downgrade rejection are complete. Short-lived session authorization (SEC4.5d) and connection/frame/rate limits (SEC4.5e) remain before broader product promotion. |
...
| Security | SEC4 | current | Close the runtime trust, egress, transport, and local-data findings recorded in the 2026-08-14 security audit. | SEC4.5c pinned Remote Coding WSS is complete. Next remaining P1 slice is SEC4.5d short-lived session authorization. |
