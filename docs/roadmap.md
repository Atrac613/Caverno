...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. The frozen structural profile covered 6/8 informed development questions and 4/8 untouched holdout questions. Stratified stable-hash sampling is closed; source selection remains No-Go. Resume only with a new predeclared scope hypothesis and new holdout, not storage. |
