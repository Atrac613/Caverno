...
| Heuristic Removal | HEU3 | next | Decide file and command completion claims from ground truth and structured self-report. | Gated on HEU2; start with the claims that `ToolOutcome` already settles. |
...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. Corrected Knowledge Object/Chunk v2, provenance-attestation v1, and fixture-root source-discovery v1 pass offline replay. Next add only an opt-in, bounded, manifest-only live shadow for one explicitly selected project; storage and production remain No-Go. |
