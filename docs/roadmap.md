...
| Local LLM | LL35 | current | Explicit goal-state tool with a real acknowledgement: replace the lexical `_looksComplete` / `_looksBlocked` goal transitions with `update_goal(completed:/blocked_reason:/message:)` whose reply carries the harness's actual verdict, and mine the plan's first unchecked checklist item as the continuation next-step. Local-first: tool-call fidelity is an LL3 probe, and user confirmation at budget exhaustion is a first-class completion path so weak models get closable goals instead of unclosable ones. | The tool/ack/shadow path is live, and `e77a57e7` repaired the observation denominator by recording agreements for every eligible active-goal turn. A clean grounded `qwen3.6-27b-vision` canary produced 4 comparisons: 3 agree, 1 known `tool_accepted_lexical_missed`, 0 unknown. Keep lexical inference authoritative while several independent goal runs accumulate; then use the distribution to gate the LL3 fidelity/confirmation slice and lexical removal. Plan-checklist next-step mining remains an additive implementation slice. |
