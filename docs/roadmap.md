...
| Watch | WATCH14 | current | Browse the iPhone's paired host projects and existing threads, read a compact conversation, and dictate instructions into the selected remote thread. | Slices 1-3 now provide paged browsing, confirmed compact transcripts, and native Dictation/Stop bound to the host connection epoch, project, and conversation with correlated accepted, queued, refused, and unknown outcomes. Next: complete the signed iPhone/Watch and real-desktop flow, including background delivery, reconnect, concurrent thread changes, accessibility, and hidden tool traffic. See [WATCH14](apple_watch_roadmap.md#watch14-remote-projects-and-voice-threads). |
