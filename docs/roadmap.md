...
| Remote Coding | RC1 | next | Add authenticated confidential transport, downgrade rejection, bounded unauthenticated connections/frames, reconnect resilience, support diagnostics, and multi-device evidence. | SEC4.5e-A connection caps and authentication deadlines are complete. Frame-size and authenticated message-rate limits remain in SEC4.5e-B before broader product promotion. |
...
| Security | SEC4 | current | Close the runtime trust, egress, transport, and local-data findings recorded in the 2026-08-14 security audit. | SEC4.5e-A connection caps and authentication deadlines are complete. Next remaining P1 slice is SEC4.5e-B frame-size and authenticated message-rate limits. |
