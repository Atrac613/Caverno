...
| Local LLM | LL37 | current | Objective verification for unattended runs only: the N-way panel runs at idle via LL18 against goals completed by routines / overnight retry-until-green / LL13 agents. No inline stage — while a user is present, LL35's confirmation rung is cheaper and more accurate than a local verifier. | Fidelity gate in progress: legacy schema-v1 Routine and LL13 pairs remain readable but are not production-eligible. The first consented schema-v2 LL13 pair scored one mechanically-green correct arm and one mechanically-green broken arm on `qwen3.6-35b-a3b-vision`, with an independent changed-file acceptance criterion, confidence 1.0, and zero invalid/unverifiable outputs. The current eligible inventory is one correct and one broken case, one objective, and one LL13 surface. The gate remains `no_go_insufficient_eligible_sample`; add four objective-distinct schema-v2 pairs including a second unattended surface, then run one combined ten-case probe before panel wiring. |
