...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. Stratified stable-hash sampling is closed. The complete caller-declared source-roots development contract passes 11/11 scope decisions, including all 12 in-scope and four out-of-scope evidence paths. An untouched holdout, production source selection, and storage remain No-Go. |
