...
| Local LLM | LL37 | current | Objective verification for unattended runs only: the N-way panel runs at idle via LL18 against goals completed by routines / overnight retry-until-green / LL13 agents. No inline stage — while a user is present, LL35's confirmation rung is cheaper and more accurate than a local verifier. | Fidelity gate in progress: the synthetic live pair remains excluded. The approved anonymized Routine pair and a consented LL13 controlled live pair both scored 2/2 on `qwen3.6-35b-a3b-vision`, for two correct and two broken cases over two eligible surfaces with zero invalid/unverifiable outputs. LL13 now persists bounded typed mutation evidence and has a consent-gated integrity-checking/redacting exporter; the known-broken canary arm records its write-tools-disabled provenance explicitly. The gate remains `no_go_insufficient_eligible_sample`; add three objective-distinct Routine/LL13 pairs and run one combined ten-case probe before panel wiring. |
