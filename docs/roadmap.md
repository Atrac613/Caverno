...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. Stratified stable-hash sampling is closed. The complete caller-declared source-roots contract is Go, and a five-root chat memory/persistence development declaration admits 451 sources with zero blockers. Evaluation questions, source selection, and storage remain No-Go. |
