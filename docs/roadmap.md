...
| Local LLM | LL37 | current | Objective verification for unattended runs only: the N-way panel runs at idle via LL18 against goals completed by routines / overnight retry-until-green / LL13 agents. No inline stage — while a user is present, LL35's confirmation rung is cheaper and more accurate than a local verifier. | Fidelity gate in progress: the synthetic live pair remains excluded. After explicit approval, one neutral, network-anonymized scheduled Routine correct/broken pair scored 2/2 on `qwen3.6-35b-a3b-vision` at confidence 1.0, with 0% false refutes, 100% broken recall, and zero unverifiable/invalid outputs. This establishes two eligible cases on one surface but remains `no_go_insufficient_eligible_sample`; collect four more correct/broken pairs including a second unattended surface before panel wiring. |
