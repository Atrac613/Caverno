...
| Watch | WATCH14 | current | Browse the iPhone's paired host projects and existing threads, read a compact conversation, and dictate instructions into the selected remote thread. | Slice 1 adds paged project/thread browsing, explicit source choice, and selection confirmed by a matching host snapshot. Next: compact remote transcripts, then destination-bound voice sending. Remote selections currently show a placeholder and input stays disabled; signed-device verification remains open. See [WATCH14](apple_watch_roadmap.md#watch14-remote-projects-and-voice-threads). |
