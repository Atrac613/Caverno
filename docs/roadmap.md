...
| Local LLM | LL10 | done | Installed-dependency grounding: resolve APIs from the project's locked dependency sources, offline. | Use `tool/run_ll10_dependency_grounding_release_gate.sh` to verify lockfile-exact source/docs grounding and future-only API rejection. |
...
Remaining `later` Local LLM milestones (LL11, LL13), Foundation F5, and
