...
| Security | SEC4 | current | Close the runtime trust, egress, transport, and local-data findings recorded in the 2026-08-14 security audit. | SA-11 is complete: default exports are secret-free, persisted credentials use platform secure storage, and explicit include-secrets file backup uses authenticated encryption with quarantine-preserving restore. Continue SEC4.6 with SA-13 nested audit redaction and owner-only filesystem modes. |
