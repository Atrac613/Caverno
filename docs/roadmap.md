...
| Local LLM | LL35 | next | Explicit goal-state tool with a real acknowledgement: replace the lexical `_looksComplete` / `_looksBlocked` goal transitions with `update_goal(completed:/blocked_reason:/message:)` whose reply carries the harness's actual verdict, and mine the plan's first unchecked checklist item as the continuation next-step. Local-first: tool-call fidelity is an LL3 probe, and user confirmation at budget exhaustion is a first-class completion path so weak models get closable goals instead of unclosable ones. | Land the tool plus the ack-carrying reply with the lexical inference still authoritative, add the fidelity probe in the same slice, and start collecting shadow-mode disagreement records. |
| Local LLM | LL36 | next | Instrument for LL37 (as LL31 was for LL29/LL30) — heuristic demotion and firing audit: label every remaining lexical guard, emit an LL33 transform record on each firing, bar guards from setting terminal state, and delete lexical paths on measured firing records instead of on argument. | Label and record `NarratedTranscriptClaimGuard` and the deferral detector first — the two with the least evidence behind them. |
