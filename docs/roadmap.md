...
| Foundation | F6 | done | Guard the built-in tool catalog against silent tool-search omissions: every built-in tool must be explicitly classified as initial-load or intentionally deferred, enforced in CI. | Exhaustiveness test over `BuiltInToolRegistry.tools` shipped; `resolve_installed_dependency` restored to the initial set. Making `ToolDefinitionSearchService` initial-load category/flag-driven (removing the hand-maintained list) is a deferred optional follow-up. |
