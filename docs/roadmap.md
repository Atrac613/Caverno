...
| Local LLM | LL34 | next | Structured tool-result envelope: give `McpToolResult` typed outcome fields (exit code, file mutations + content hashes, read content hashes, test counts) so first-party tool facts survive the tool boundary, and rewire the failure classifiers and claim guards to read facts instead of re-parsing result strings. Census-backed: 8 tools cover 67.6% of real tool traffic, and the `read_file` hash turns the measured dominant failure (redundant re-reads, ~53% of sessions) into a field comparison. | Exit-status half shipped (`ToolOutcome` contract, `McpToolResult.outcome`, `ToolFailureClassifier` reading it, producers for `git_execute_command` / `git_finish_worktree_session` / `local_execute_command` — 26.2% of measured traffic). Next: build a lift point for `read_file`'s content hash and rewire `ToolLoopContextDigest` onto it. |
| Local LLM | LL35 | current | Explicit goal-state tool with a real acknowledgement: replace the lexical `_looksComplete` / `_looksBlocked` goal transitions with `update_goal(completed:/blocked_reason:/message:)` whose reply carries the harness's actual verdict, and mine the plan's first unchecked checklist item as the continuation next-step. Local-first: tool-call fidelity is an LL3 probe, and user confirmation at budget exhaustion is a first-class completion path so weak models get closable goals instead of unclosable ones. | First slice shipped and measured: `update_goal` contract + runtime adapter, approval-gated in `chat_tool_handler_catalog`, `goal_completion_shadow.dart` collecting disagreements, and the sample already read (`731adfcf`). Not yet built: the LL3 call-fidelity probe and the plan-checklist next-step mining; lexical inference is still authoritative by design at this stage. |
| Local LLM | LL36 | current | Instrument for LL37 (as LL31 was for LL29/LL30) — heuristic demotion and firing audit: label every remaining lexical guard, emit an LL33 transform record on each firing, bar guards from setting terminal state, and delete lexical paths on measured firing records instead of on argument. | Started: the firing measurement landed (`0644c72c`, which lexical guards actually fire) and `NarratedTranscriptClaimGuard` exists. Remaining: stable pattern labels, an LL33 transform record per firing, and barring guards from terminal state. |
| Local LLM | LL37 | later | Objective verification for unattended runs only: the N-way panel runs at idle via LL18 against goals completed by routines / overnight retry-until-green / LL13 agents. No inline stage — while a user is present, LL35's confirmation rung is cheaper and more accurate than a local verifier. Uncertainty defaults to *not* refuted below an LL3 fidelity threshold. | Gated on LL36's measurements, not just its landing. Harvest the convergence controls (anti-ratchet, stall exit, blocking classification) into the LL7 retry loop independently — that slice survives even if this milestone is dropped. |
| Local LLM | LL38 | done | Mid-turn interruption (steering): a message typed while a turn is running joins that turn instead of waiting behind it, opt-in per send (`sendMessage(interrupt: true)`) so the queue keeps its owner-receipt contract. The interaction generation never advances, so the turn keeps its owner, tool results and partial output and no partial-response recovery path fires; an interruption no request ever carried is handed back to the queue at turn teardown. | Live-verified 5/5 on `qwen3.6-35b-a3b-vision` with a queued control arm (`tool/run_turn_steering_live_canary.sh`); delivery and obedience are asserted separately. Not covered: interrupting a plain tool-free answer, which has no later request to carry the message and so degrades to the queue by design. |
