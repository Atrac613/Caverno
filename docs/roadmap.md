...
| Watch | WATCH14 | next | Browse the iPhone's paired host projects and existing threads, read a compact conversation, and dictate instructions into the selected remote thread. | Requested 2026-09-14. Start with bounded project/thread navigation and explicit source selection, then compact transcripts, destination-bound voice sending, and signed-device verification. Hide tool calls, raw results, and reasoning from the transcript while keeping approvals/questions reachable. WATCH11 covers only remote interactions; WATCH5's push matrix remains a separate gate. See [WATCH14](apple_watch_roadmap.md#watch14-remote-projects-and-voice-threads). |
