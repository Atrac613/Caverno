...
| Remote Coding | RC1 | next | Add authenticated confidential transport, downgrade rejection, bounded unauthenticated connections/frames, reconnect resilience, support diagnostics, and multi-device evidence. | SEC4.5c pinned WSS and SEC4.5d challenge-bound sessions are complete. Connection/frame/rate limits (SEC4.5e) remain before broader product promotion. |
...
| Security | SEC4 | current | Close the runtime trust, egress, transport, and local-data findings recorded in the 2026-08-14 security audit. | SEC4.5d challenge-bound Remote Coding sessions are complete. Next remaining P1 slice is SEC4.5e connection/frame/rate limits. |
