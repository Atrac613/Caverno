...
| Local LLM | LL35 | done | Explicit goal-state tool with a real acknowledgement: goal terminal state now comes from a finally reconciled `update_goal(completed:/blocked_reason:/message:)`, all-completed structured tasks, or the user-confirmation path; lexical completion/blocker prose remains shadow-only. Continuations prefer the typed active task and otherwise mine the first unchecked bounded `## Task checklist` item. Local-first: tool-call fidelity is an LL3 probe, and per-model `tool` / `tool_or_ask` / `ask` policy makes budget/no-work confirmation a first-class completion path. | Tool/ack/owner-safe finalization, denominator logging, next-step selection, fidelity profile storage, per-model settings/UI, confirmation summaries/actions, blocker/paused-cap persistence, and lexical terminal demotion are complete. Three clean TODO sessions contributed 13 comparisons (10 agree, 3 explained tool-accepted/lexical-missed, 0 unknown). A distinct Markdown TOC goal added 2 agree / 0 unknown comparisons but failed its overall task because a repair was not revalidated; the goal correctly remained blocked. A direct `qwen3.6-27b-vision` probe returned exactly `update_goal({"completed":true})`, establishing a `reliable` observed fidelity verdict. Continue shadow monitoring as operations, not remaining implementation. |
| Local LLM | LL36 | current | Instrument for LL37 (as LL31 was for LL29/LL30) — heuristic demotion and firing audit: label every remaining lexical guard, emit an LL33 transform record on each firing, bar guards from setting terminal state, and delete lexical paths on measured firing records instead of on argument. | Additive instrumentation and structural enforcement are complete, including stable final-answer labels, generation-owned recording, nested-loop transform preservation, and a clean pending-action recovery canary on `qwen3.6-27b-vision` (`4c0efc96`, 1/1 ready). Remaining: accumulate a model-varied post-provenance corpus, then remove at least one lexical path only when its grounded disagreement record supports deletion. |
