...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. The opt-in manifest-shadow boundary passes temporary-repository coverage, while the Caverno live preflight is No-Go at 2,816 candidates versus its 2,048 hard ceiling. Measure metadata-only top-level source scopes next; do not raise the cap or start storage. |
