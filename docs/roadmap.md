...
| Local LLM | LL34 | next | Structured tool-result envelope: preserve producer-owned command, filesystem, process, diagnostic, and verification facts across the tool boundary; prefer those facts in guards and compact weak-model prompts while keeping outcome-free third-party MCP results on a measured lexical fallback. | Implementation ships end to end: rich outcomes, direct typed first-party producers, typed-first consumers with verdict provenance, current-turn mutation-backed file claims, session-log shadow markers, and an LL23 per-model summary-first toggle. The measurement fixture reduces estimated prompt tokens by 62.8%. Rollout remains gated because no fresh grounded post-change coding log contains the marker yet; collect one in a new session-log directory, classify every missing/mismatch case, then opt a measured model into summary-first. |
