...
| Retrieval | RAG2 | later | Index active-project code and Markdown as provenance-bearing Knowledge Objects in additive drift/SQLite + FTS5 storage. | The investigation is preserved in `docs/rag2_investigation_handoff_2026-08-25.md`. Stratified stable-hash sampling is closed. Complete caller-declared source roots are promotion Go; CI reruns Git-backed acquisition for both frozen declarations. Storage semantics are Go in a backend-neutral in-memory replay. Retrieval, persistence-backend selection, and production remain No-Go. |
