...
| Local LLM | LL34 | next | Structured tool-result envelope: give `McpToolResult` typed outcome fields (exit code, file mutations + content hashes, read content hashes, test counts) so first-party tool facts survive the tool boundary, and rewire the failure classifiers and claim guards to read facts instead of re-parsing result strings. Census-backed: 8 tools cover 67.6% of real tool traffic, and the `read_file` hash turns the measured dominant failure (redundant re-reads, ~53% of sessions) into a field comparison. | Populate the envelope for `local_execute_command` (exit code) and `read_file` (content hash) first — 43% of traffic for two tools — then rewire `ToolFailureClassifier`, then `ToolLoopContextDigest`. |
