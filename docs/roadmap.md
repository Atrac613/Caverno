...
| Local LLM | LL29 | next | Tool-loop failure recovery: degrade gracefully on repeated tool failures instead of aborting the whole turn (inject a recovery hint and keep iterating; hard halt is opt-in). | Rewire the `toolFailureCounts` branch in `chat_notifier.dart` from `break` to graded warn/halt with action-oriented hints; land with LL31. |
| Local LLM | LL30 | next | Compaction structural pre-pass: dedupe + one-line-summarize old tool results and token-budget the protected tail before summarizing. | Build the pure prune/summary helper beside `ConversationCompactionService` and measure token savings in the LL14 style. |
| Local LLM | LL31 | next | Turn-exit reason + completion explainer: tag every loop exit and replace blank/partial responses with a "why it stopped" explanation. | Thread a `turnExitReason` through the loop break sites and add a post-loop explainer + mid-work WARNING log. |
