...
| Local LLM | LL37 | current | Objective verification for unattended runs only: the N-way panel runs at idle via LL18 against goals completed by routines / overnight retry-until-green / LL13 agents. No inline stage — while a user is present, LL35's confirmation rung is cheaper and more accurate than a local verifier. | Fidelity gate in progress: the synthetic live pair remains excluded. The approved anonymized Routine pair scored 2/2 on `qwen3.6-35b-a3b-vision`, establishing two eligible cases on one surface. Second-surface inventory found no saved LL7/LL13 runs and exposed missing LL13 changed-file evidence; bounded typed-mutation capture now persists relative paths, hashes, sizes, deletion/truncation state, and content with backward-compatible defaults. The gate remains `no_go_insufficient_eligible_sample`; record a consented LL13 correct/broken pair next, then fill the remaining 5+5 denominator before panel wiring. |
