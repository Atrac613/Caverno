...
| Local LLM | LL34 | next | Structured tool-result envelope: give `McpToolResult` typed outcome fields (exit code, mutated files + content hashes, test counts) so first-party tool facts survive the tool boundary, and rewire the failure classifiers and claim guards to read facts instead of re-parsing result strings. Lead milestone of the Grounded Verification Track — deletes regex rather than adding heuristics, so it is not gated on LL31 triage. | Define the envelope beside `McpToolResult`, populate exit codes in `local_shell_tools` and mutations+hashes in the filesystem write path, then rewire `ToolFailureClassifier` as the first consumer. |
...
| Local LLM | LL36 | next | Heuristic demotion and firing audit: label every remaining lexical guard, emit an LL33 transform record on each firing, bar guards from setting terminal state, and delete lexical paths on measured firing records instead of on argument. | Label and record `NarratedTranscriptClaimGuard` and the deferral detector first — the two with the least evidence behind them. |
| Local LLM | LL37 | later | Objective verification split by cost: a single cheap verifier inline on a completion claim (bounded, fails open) and the expensive N-way panel at idle via LL18, with the convergence controls that make it terminate. Local-first inversions vs Grok Build: uncertainty defaults to *not* refuted for weak verifiers, an LL3 fidelity gate disables the stage below threshold, and the strategist is idle-only. | Gated on LL34-LL36. Settle "is a local verifier good enough at all" with LL19 replay (false refutes on correct work vs recall on broken work) before building; ship idle-only or drop if false refutes dominate. |
