...
| Local LLM | LL35 | current | Explicit goal-state tool with a real acknowledgement: replace the lexical `_looksComplete` / `_looksBlocked` goal transitions with `update_goal(completed:/blocked_reason:/message:)` whose reply carries the harness's actual verdict, and mine the plan's first unchecked checklist item as the continuation next-step. Local-first: tool-call fidelity is an LL3 probe, and user confirmation at budget exhaustion is a first-class completion path so weak models get closable goals instead of unclosable ones. | The tool/ack/shadow path is live, and `e77a57e7` repaired the observation denominator. Three independent `qwen3.6-27b-vision` goal sessions now provide 13 comparisons: 10 agree, 3 known `tool_accepted_lexical_missed`, 0 unknown; `89c6cb85` fixed a canary false negative exposed by the second run. Keep lexical inference authoritative and defer the LL3 confirmation path while `goal_completion_lexical_only` remains zero. Next additive code slice: bounded plan-checklist next-step mining while another goal shape broadens the evidence. |
