...
| Local LLM | LL34 | next | Structured tool-result envelope: give `McpToolResult` typed outcome fields (exit code, file mutations + content hashes, read content hashes, test counts) so first-party tool facts survive the tool boundary, and rewire the failure classifiers and claim guards to read facts instead of re-parsing result strings. Census-backed: 8 tools cover 67.6% of real tool traffic, and the `read_file` hash turns the measured dominant failure (redundant re-reads, ~53% of sessions) into a field comparison. | All scoped typed facts now ship: exit status, read identity, mutation change, diagnostic counts, process lifecycle, and verification test counts. `CodingVerificationClaimGuard` is typed-first with evidence JSON fallback. `ToolOutcomeShadow` exit-code comparisons now persist as owner-scoped session-log markers, and triage reports their agreement distribution without affecting anomaly scores. Next: collect a fresh grounded coding sample and understand every disagreement before summary-first rendering. |
