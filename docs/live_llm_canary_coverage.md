...
affecting the bounded score. A capability-only run is valid even though its
attempted score denominator is zero; the canary still requires at least one
diagnostic probe or sampler block to have actually run.
...
4. Dart TODO app MVP assembly check:
   ...
   `dart run tool/verify_todo_app.dart` command. Every verification runs in a
   fresh temporary copy containing only the Dart sources and `pubspec.yaml`, so
   model-chosen runtime state filenames cannot leak between checks. Any write or
   edit attempted after the first successful verifier result is blocked and fails readiness with
   ...
   To measure autonomous specification reading with the concise production-like
   Japanese prompt instead of the detailed builder instructions, run:
   ...
   This run sends the same short-prompt shape for
   ...
   This short-prompt run uses
   `docs/coding_mvp_fixtures/expense_tracker.md`. Its isolated verifier checks
...
### 2026-09-12: `qwen3.8-27b-vision` Anabasis Acceptance End To End

One run of `live_anabasis_acceptance` (session `ad2dbe83`) fired
`anabasis_delegation_admitted`, `delegated_results_named` and
`anabasis_acceptance_recorded` together, the last of which had never fired on any
build. The parent delegated a ready saved task, read the child back by the id the
delegated-results block named, judged it, and wrote `accepted_task_id` with
`evidence: ["child summary recorded"]`.

```bash
CAVERNO_ANABASIS_SCENARIO=live_anabasis_acceptance \
CAVERNO_LLM_BASE_URL=http://192.168.100.241:1234/v1 \
CAVERNO_LLM_API_KEY=no-key \
CAVERNO_LLM_MODEL=qwen3.8-27b-vision \
tool/with_live_llm_loopback.sh -- tool/run_anabasis_delegation_live_canary.sh
```

Six earlier runs on the delegation scenario recorded no acceptance, and the reason
was the task rather than the parent: an implementation is not finished after one
child call, so the parent kept working instead of judging. A judgement cannot be
observed on work that is not done. The two scenarios differ only in the goal.
